...
책을 읽는 동안 메모리에서 그래픽이 자동으로 렌더링되는 것을 완전히 방지하기 위해, 모든 그래픽 자료, 설치 스크린샷 및 GUI 설정은 본문 외부에 격리된 `_assets/images` 디렉토리로 이동되었습니다. 그래픽은 하위 폴더별로 구조화되어 있습니다. 또한 `_assets/icons` 디렉토리에는 스타일 이콘이 추가되었으며, 여기에는 `256x256` 및 `256x256@2x` 하위 폴더와 함께 내부 휴지통 스타일 이콘을 위한 하위 폴더가 포함된 `Trash` 하위 폴더가 있습니다. 마찬가지로 `_assets/wallpapers`에도 스타일리시한 배경화면이 하위 폴더별로 분류되어 저장되어 있습니다.
